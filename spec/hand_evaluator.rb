...
describe 'Hand Evaluator' do
  ...
  xit 'evals as two pair when pair on board' do
    hand = [
      {suit: :h, tag: :'J', rank: 11},
      {suit: :c, tag: :'A', rank: 14}
    ]
    ...
    result = @handEvaluator.evalHand(board, hand, @madeHands)
    #result[:pair_plus_flush_draw].should_not == ["AJch"]
    #result[:two_pair].should == ["AJch"]
    #result[:combo_draw].should == ["AJch"]
    #not sure above behavior is correct?
    #it evals as 2 pair even though the lower pair is on the board (do we want this? this is a unique
    #situation compared to having a real 2 pair
    #also it ranks as a combo draw because of the gutshot but not pair plus flush draw
  end
  ...
  it 'evals two pair' do
    hand = [
      {suit: :c, tag: :'9', rank: 9},
      {suit: :c, tag: :Q, rank: 12}
    ]
    board = [
      {suit: :h, tag: :Q, rank: 12},
      {suit: :d, tag: :"8", rank: 8},
      {suit: :h, tag: :'9', rank: 9},
      {suit: :s, tag: :'2', rank: 2}
    ]
    result = @handEvaluator.evalHand(board, hand, @madeHands)
    result[:two_pair].should == ["Q9cc"]
  end
end
